...
@onready var sciquest_picture : TextureRect = $PanelContainer/MarginContainer/VBoxContainer/HBoxContainer/TextureRect/MarginContainer/SciquestPicture
...
func _process(_delta):
	tick += 1
	if tick % 10 == 0:
		sciquest_picture.texture.region.position.x = wrapi(sciquest_picture.texture.region.position.x + 22, 0, 132)
		tick = 0
